...
function updateGroundAndBackTrees(difficulty) {
    groundOffsetX = (groundOffsetX - 2) % 1920;
    backTrees = backTrees.filter(t => (t.left -= 1.8) > -t.sprite.width);
    let lastTree = backTrees[backTrees.length - 1];
    if(Math.random() < 0.15 && (lastTree.left + lastTree.sprite.width) < (750 + 200 * difficulty)) {
        backTrees.push({
            left: 800, bottom: 30, front: Math.random() < 0.5, sprite: Math.random() < 0.5 ? firstBackTree : secondBackTree
        });
    }
}
...
function prepareBackground(canvasWidth, canvasHeight) {
    ...
    drawBackGround = prepareBackgroundDraw(back, canvasWidth, canvasHeight, canvasHeight - groundHeight - 70 - back.height);
    let middle = {
        spriteX: 301,
        spriteY: 94,
        spriteWidth:128,
    ...
    drawMiddleGround = prepareBackgroundDraw(middle, canvasWidth, canvasHeight, canvasHeight - groundHeight - middle.height + 10);
    let front = {
        spriteX: 230,
        spriteY: 54,
        spriteWidth: 30,
    ...
    drawFrontGround = prepareBackgroundDraw(front, canvasWidth, canvasHeight, canvasHeight - front.height);
}
